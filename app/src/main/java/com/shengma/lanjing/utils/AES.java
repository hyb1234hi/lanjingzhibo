...
public class AES {
    ...
    private static String ivParameter = "lanjing_lanjing_";// 密钥默认偏移，可更改
    // private byte[] iv = { 0xA, 1, 0xB, 5, 4, 0xF, 7, 9, 0x17, 3, 1, 6, 8,
    // 0xC,
    // 0xD, 91 };
    private byte[] iv = ivParameter.getBytes();
    private IvParameterSpec IV;
    String sKey = "lanjing_lanjing_";// key必须为16位，可更改为自己的key
    ...
    public String decrypt(String ciphertext_base64) {
        ...
        Log.d("AES", "skforAES:" + skforAES);

        String decrypted = new String(decrypt(skforAES, IV,
                s));
        return decrypted;
    }
    ...
    private byte[] decrypt(SecretKey sk, IvParameterSpec IV,
                           byte[] ciphertext) {
        try {
            ...
            Log.e("AESdemo", "no cipher getinstance support for " + "AES/CBC/PKCS5Padding");
        } catch (NoSuchPaddingException nspe) {
            Log.e("AESdemo", "no cipher getinstance support for padding " + "AES/CBC/PKCS5Padding");
        } catch (InvalidKeyException e) {
            Log.e("AESdemo", "invalid key exception");
        } catch (InvalidAlgorithmParameterException e) {
            Log.e("AESdemo", "invalid algorithm parameter exception");
