package com.example.myapplication2.liveroom;
...
import com.example.myapplication2.liveroom.roomutil.commondef.LoginInfo;
import com.example.myapplication2.liveroom.roomutil.commondef.MLVBCommonDef;
...
public abstract class MLVBLiveRoom {
     ...
     *
     * @param op 执行动作，定义请查看 { MLVBCommonDef.CustomFieldOp}
     * @param key 自定义键
     * @param value 数值
     * @param callback  设置自定义信息完成的回调
     *
     * @note - op 为 { MLVBCommonDef.CustomFieldOp#SET} 时，value 可以是 String 或者 Integer 类型
	 ...
	 * 2. 【主播】会收到 { IMLVBLiveRoomListener#onRequestJoinAnchor(AnchorInfo, String)} 的回调通知。
	 * 3. 【主播】调用 responseJoinAnchor() 确定是否接受观众的连麦请求。
	 * 4. 【观众】会收到 { IMLVBLiveRoomListener.RequestJoinAnchorCallback} 回调通知，可以得知请求是否被同意。
	 * 5. 【观众】如果请求被同意，则调用 startLocalPreview() 开启本地摄像头，如果 App 还没有取得摄像头和麦克风权限，会触发 UI 提示。
	 * 6. 【观众】然后调用 joinAnchor() 正式进入连麦状态。
     * 7. 【主播】一旦观众进入连麦状态，主播就会收到 { IMLVBLiveRoomListener#onAnchorEnter(AnchorInfo)} 通知。
     ...
     *
     * @see { IMLVBLiveRoomListener#onRequestJoinAnchor(AnchorInfo, String)}
     */
     ...
     *
     * @see { IMLVBLiveRoomListener#onRequestRoomPK(AnchorInfo)}
     */
     ...
     * 响应跨房 PK 请求
     *
     * 主播响应其他房间主播的 PK 请求，发起 PK 请求的主播会收到 { IMLVBLiveRoomListener.RequestRoomPKCallback} 回调通知。
     *
     ...
     * 退出跨房 PK
	 *
	 * 当两个主播中的任何一个退出跨房 PK 状态后，另一个主播会收到 { IMLVBLiveRoomListener#onQuitRoomPK(AnchorInfo)} 回调通知。
     *
